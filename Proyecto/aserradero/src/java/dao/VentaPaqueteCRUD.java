...
//    }
    ...
    public <T> List listar(String id_jefe) throws Exception {
        ...
        try {
            this.abrirConexion();
            try (PreparedStatement st = this.conexion.prepareStatement("SELECT * FROM VISTA_VENTA_PAQUETE")) {
    ...
    public <T> List buscar(String nombre_campo, String dato) throws Exception {
        ...
        try {
            this.abrirConexion();
            try (PreparedStatement st = this.conexion.prepareStatement("SELECT * FROM VISTA_VENTA_PAQUETE WHERE " + nombre_campo + " like ?")) {
                st.setString(1, "%" + dato + "%");
                ventaPaquetes = new ArrayList();
                try (ResultSet rs = st.executeQuery()) {
                    while (rs.next()) {
