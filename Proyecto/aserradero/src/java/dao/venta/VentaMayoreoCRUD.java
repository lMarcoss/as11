...
public class VentaMayoreoCRUD extends Conexion implements OperacionesCRUD {
    ...
    public void registrar(Object objeto) throws Exception {
        ...
        try {
            ...
            PreparedStatement st = this.conexion.prepareStatement(
                    "INSERT INTO VENTA_MAYOREO (id_venta,id_madera,num_piezas,volumen,monto,tipo_madera) VALUES (?,?,?,?,?,?)");
    ...
    public <T> List listar(String id_jefe) throws Exception {
        ...
        try {
            this.abrirConexion();
            try (PreparedStatement st = this.conexion.prepareStatement("SELECT * FROM VENTA_MAYOREO WHERE id_jefe = ?")) {
    ...
    public Object modificar(Object objeto) throws Exception {
        ...
        this.abrirConexion();
        try (PreparedStatement st = this.conexion.prepareStatement("SELECT * FROM VENTA_MAYOREO WHERE id_venta = ? AND id_madera = ? AND (id_venta NOT IN (SELECT id_venta FROM VENTA WHERE estatus = 'Pagado'))")) {
    ...
    public void actualizar(Object objeto) throws Exception {
        ...
        try {
            this.abrirConexion();
            PreparedStatement st = this.conexion.prepareStatement("UPDATE VENTA_MAYOREO SET num_piezas = ?, volumen = ?, monto = ?, tipo_madera = ? WHERE id_venta = ? AND id_madera = ?");
            st.setInt(1, ventaMayoreo.getNum_piezas());
            st.setFloat(2, ventaMayoreo.getVolumen());
            st.setFloat(3, ventaMayoreo.getMonto());
            st.setString(4, ventaMayoreo.getTipo_madera());
            st.setString(5, ventaMayoreo.getId_venta());
            st.setString(6, ventaMayoreo.getId_madera());
            st.executeUpdate();
        } catch (Exception e) {
            System.out.println(e);
            throw e;
    ...
    public <T> List buscar(String nombre_campo, String dato, String id_jefe) throws Exception {
        ...
        try {
            this.abrirConexion();
            try (PreparedStatement st = this.conexion.prepareStatement("SELECT * FROM VENTA_MAYOREO WHERE " + nombre_campo + " like ? AND id_jefe = ?")) {
                st.setString(1, "%" + dato + "%");
