...
public class PagoEmpleadoCRUD extends Conexion implements OperacionesCRUD{
    ...
    public <T> List listar() throws Exception{
        ...
        try{
            this.abrirConexion();
            try (PreparedStatement st = this.conexion.prepareStatement("SELECT * FROM VISTA_PAGO_EMPLEADO")) {
    ...
    public <T> List buscar(String nombre_campo, String dato) throws Exception{
        ...
        try{
            this.abrirConexion();
            try (PreparedStatement st = this.conexion.prepareStatement("SELECT * FROM VISTA_PAGO_EMPLEADO WHERE "+nombre_campo+" like ?")) {
                st.setString(1, "%"+dato+"%");
                pagoEmpleadoes = new ArrayList();
                try (ResultSet rs = st.executeQuery()) {
                    while (rs.next()) {
