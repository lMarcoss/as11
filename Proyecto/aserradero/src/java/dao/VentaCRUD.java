...
public class VentaCRUD extends Conexion implements OperacionesCRUD{
    ...
    public <T> List listar() throws Exception {
        ...
        try{
            this.abrirConexion();
            try (PreparedStatement st = this.conexion.prepareStatement("SELECT * FROM VENTA ORDER BY estatus,tipo_venta")) {
    ...
    public <T> List buscar(String nombre_campo, String dato) throws Exception {
        ...
        try{
            this.abrirConexion();
            try (PreparedStatement st = this.conexion.prepareStatement("SELECT * FROM VENTA WHERE "+nombre_campo+" like ?")) {
                st.setString(1, "%"+dato+"%");
                ventas = new ArrayList();
                try (ResultSet rs = st.executeQuery()) {
                    while (rs.next()) {
    ...
    public <T> List listarVentasExtras() throws Exception {
        List<Venta> ventas;
        try{
            this.abrirConexion();
            try (PreparedStatement st = this.conexion.prepareStatement("SELECT * FROM VENTA WHERE tipo_venta = ? AND estatus = ?")) {
    ...
    public <T> List listarVentasPaquete() throws Exception {
        List<Venta> ventas;
        try{
            this.abrirConexion();
            try (PreparedStatement st = this.conexion.prepareStatement("SELECT * FROM VENTA WHERE tipo_venta = ? AND estatus = ?")) {
    ...
    public <T> List listarVentasMayoreo() throws Exception {
        List<Venta> ventas;
        try{
            this.abrirConexion();
            try (PreparedStatement st = this.conexion.prepareStatement("SELECT * FROM VENTA WHERE tipo_venta = ? AND estatus = ?")) {
