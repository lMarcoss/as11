...
BEGIN
	...
	IF EXISTS (SELECT id_persona FROM CUENTA_POR_COBRAR WHERE id_persona = id_cliente_a) THEN
		...
		IF (SELECT monto FROM CUENTA_POR_COBRAR WHERE id_persona = id_cliente_a) - monto_anticipo_a < 0 THEN
			INSERT INTO CUENTA_POR_PAGAR SET
				CUENTA_POR_PAGAR.id_persona = id_cliente_a,
				CUENTA_POR_PAGAR.monto = ABS((SELECT monto FROM CUENTA_POR_COBRAR WHERE id_persona = id_cliente_a) - monto_anticipo_a)
			ON DUPLICATE KEY UPDATE 
				CUENTA_POR_PAGAR.monto= CUENTA_POR_PAGAR.monto+ABS((SELECT monto FROM CUENTA_POR_COBRAR WHERE id_persona = id_cliente_a) - monto_anticipo_a);
            
			DELETE FROM CUENTA_POR_COBRAR WHERE id_persona = id_cliente_a;
		ELSE -- y si no es menor entonces se resta 
			UPDATE CUENTA_POR_COBRAR SET CUENTA_POR_COBRAR.monto = CUENTA_POR_COBRAR.monto - monto_anticipo_a WHERE CUENTA_POR_COBRAR.id_persona = id_cliente_a;
		END IF;
        
        -- eliminamos la cuenta del cliente en cuentas por cobrar si es cero
        IF (SELECT monto FROM CUENTA_POR_COBRAR WHERE id_persona = id_cliente_a) = 0 THEN
			DELETE FROM CUENTA_POR_COBRAR WHERE id_persona = id_cliente_a;
		END IF;
	...
	ELSE -- si el cliente no debe dinero entonces se inserta como cuenta por pagar
		INSERT INTO CUENTA_POR_PAGAR SET
				CUENTA_POR_PAGAR.id_persona = id_cliente_a,
				CUENTA_POR_PAGAR.monto = monto_anticipo_a
			ON DUPLICATE KEY UPDATE 
				CUENTA_POR_PAGAR.monto= CUENTA_POR_PAGAR.monto + monto_anticipo_a;
    END IF;
END;//
DELIMITER ;
